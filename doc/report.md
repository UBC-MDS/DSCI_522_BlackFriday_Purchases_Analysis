...
Black Friday is a name given to the shopping day after Thanksgiving in
the United States. Many people go out to shop and spend a large amount
of money. People might be curious about how male’s purchase behavior on
Black Friday differs from that of female. One specific question to ask
is whether male and female spend the same amount on Black Friday. In
this project, we will apply data analysis techniques to find an answer
to this question.
...
about $5,000 to $13,000. The mean purchase amount is about $9334.

![](../imgs/purch_dist.png)

*Figure 2. The histogram plot of all purchases made on Black Friday. The
red vertical line represents the mean purchase amount on Black Friday.*
...
addition, the difference between the means of these two groups look
quite small on the graph. Does this imply that male and female spent the
same amount on Black Friday? Let’s find out in our formal analysis.

![](../imgs/gender_pur_hist.png)

*Figure 3. These histograms show little difference between the mean
purchase amount spent by male and female. The red vertical lines
represent the mean purchases amount made by each gender.*

# 4.0 Analysis

Our analysis begins with the definition of null hypothesis and
alternative hypothesis. The null hypothesis \(H_0\) is that the mean
purchase amount made by male is not different from the mean purchase
amount made by female, while the alternative hypothesis \(H_A\) is that
the mean purchase amount made by male is different than the mean
...
of the population. The results of the estimations are summarized
below.
...
*Table 3. The estimated population mean purchase amounts with 95%
confidence interva.*
...
*Figure 4. The red points represent the mean purchase amount. The error
...
*Figure 5. The t-distribution with t-statistic. The blue dashed lines
...
In this project, we base our statistical inference on the asymptotic
theory. It means we assume the sample used in this project meets the two
important conditions of the CLT.

First, the sample is independent. To meet the independence condition, we
consider two things.  
...
Second, the sample size is big enough. Since the sample contains more
...
Firstly, we would improve the randomness of the sample. Our current
sample includes only sales records from one store. It is prone to be
biased. One way to improve this is to randomly select purchase records
